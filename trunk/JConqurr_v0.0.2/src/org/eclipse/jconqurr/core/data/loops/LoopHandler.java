...
public class LoopHandler implements Runnable {
	...
	/**
	 * Constructs a LoopHandler that sets the start and end parameters to the
	 * range and sets the number of threads. The constructor will also
	 * initialize the loop range, set IForLoopTask loopBody as null and created
	 * a fixed thread pool with the given number of threads
	 * 
	 * @param start
	...
	private synchronized LoopRange getTaskRange() {
		...
		result.setEnd((currentBoundary < range.getEnd()) ? currentBoundary
				: range.getEnd());
		return result;
	}
	...
	private void processLoop() {
		...
		while ((currentTaskRange = this.getTaskRange()) != null) {
			LoopTask loopTask = new LoopTask(currentTaskRange.getStart(),
					currentTaskRange.getEnd(), loopTaskBody);
