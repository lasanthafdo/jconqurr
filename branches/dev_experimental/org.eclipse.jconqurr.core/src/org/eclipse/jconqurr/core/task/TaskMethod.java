		...
		if (taskStartPositions.size() > 0) {
			...
			for (int m = 0; m < taskStartPositions.size(); m++) {
				...
				if (m + 1 < taskStartPositions.size()) {
					/*System.out.println(taskStartPositions.get(m + 1));
					System.out.println(taskEndPositions.get(m));
					System.out.println(stmts.subList(
							taskEndPositions.get(m) + 1, taskStartPositions
									.get(m + 1)));*/
